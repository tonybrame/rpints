            ...
            <tbody>
                <?php foreach($activeTaps as $tap){ ?>
                    ...
                    ?>
						...
						<td>
                            <input name="tareScale<?php echo $tap->get_id(); ?>" type="button" class="btn" value="Tare" onClick="tareScale(this, <?php echo $tap->get_tapNumber(); ?>)" />
                        </td>
...
<?php } ?>
	...
	async function tareScale(btn, tapNumber){
		alert("Will send tare 10 seconds after clicking ok... ");
		sleep(10000).then(() => {
		        var mySocket = new WebSocket("ws://192.168.5.98:3005/tare");
			mySocket.addEventListener("open", () => {
				mySocket.send(tapNumber);
				alert("success...");
				mySocket.close();
			});
		});
